    ...
    <div class="container">
        ...
        <table class="table table-striped" id="datatable">
            ...
            <c:forEach items="${users}" var="user">
                ...
                <tr id="${user.id}" data-userEnabled="<c:if test="${!user.enabled}">false</c:if>">
                    <td><c:out value="${user.name}"/></td>
                    <td><a href="mailto:${user.email}">${user.email}</a></td>
                    <td>${user.roles}</td>
                    <td><input type="checkbox" onclick="enable($(this).closest('tr').attr('id'), $(this).is(':checked'))" <c:if test="${user.enabled}">checked</c:if>/></td>
